...
def test_response_function_calculation(generate_s1_gate, generate_ptet):
    ...
    assert s1.calculate_response_function() == \
           pytest.approx(0.0030, 0.1)
    # Now we validate a low signal.
    s1.set_logical_inputs([0b1111])
    assert s1.calculate_response_function() == \
           pytest.approx(1.2965, 0.1)
...
def test_circuit_score(
        generate_s1_gate,
        generate_p1_gate, generate_plux_star,
        generate_ptet,
):
    s1 = generate_s1_gate
    ptet = generate_ptet
    ...
    p1.set_biological_inputs([plux, s1])
    p1.set_logical_function('NOR')
    assert p1.score_self() == \
           pytest.approx(2.3326, 0.1)
